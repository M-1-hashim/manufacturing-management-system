...
Task ID: 21
Agent: main (Z.ai Code)
Task: ساخت فایل HTML تک‌فایلی دمو از کل سیستم برای ارائه به مشتریان

Work Log:
- بررسی طراحی واقعی: globals.css (تم زمردی oklch hue 165)، page.tsx (۱۳ ماژول)، public/fonts/B-Nazanin.ttf (۶۰KB قابل تعبیه)
- ساخت public/demo.html (۱۶۸KB تک‌فایل مستقل — بدون انترنت/سرور/نصب):
  * فونت B Nazanin تعبیه‌شده base64 + RTL کامل + تم زمردی شبیه برنامه اصلی
  * صفحه ورود با ورود سریع دمو (admin/admin123)
  * ۱۳ ماژول کامل: داشبورد (۸KPI + چارت سطحی روند فروش + باره گروهی تولید + کم‌موجودها)، فروش POS تعاملی (سبد، تخفیف، مالیات ۲/۱۰٪، ثبت فروش واقعی)، تولید (شروع/تکمیل با به‌روزرسانی انبار)، محصولات (جستجو/فیلتر/ویرایش)، مواد خام، فرمولاسیون BOM با محاسبه هزینه، انبار، مالی (دونات مصارف + روند سود)، کارکنان، گزارشات، کاربران RBAC، فعالیت‌ها، تنظیمات (۱۱ تم رنگی کارا + شب/روز)
  * چارت‌ها SVG دست‌ساز (بدون کتابخانه خارجی)، آیکون‌های Lucide درون‌خطی، ماه‌های افغانی
  * دیتای نمونه واقع‌گرایانه: کارخانه رنگ‌سازی کابل (۸ محصول، ۸ ماده اولیه، ۳ فرمول، ۵ سفارش تولید، فکتورها، مصارف، کارکنان)
  * ریسپانسیو (سایدبار off-canvas موبایل) + چاپ + بنر «نسخه نمایشی» + watermark
- فیکس‌ها: دو باگ نحوی JS (رشته ویرایش)، typo دیتا، باگ ALTER تکراری، پولیش CSS موبایل
- تست agent-browser: لاگین ✓، هر ۱۳ ماژول رندر بدون خطا ✓، ثبت فروش (بل INV-1404-124 + موجودی ۲۱۴→۲۱۲ + لاگ انبار/حسابرسی) ✓، تکمیل تولید (+۲۵۰ انبار) ✓، ۱۱ تم + دارک ✓، موبایل ۳۹۰px + منو ✓، بدون خطای کنسول ✓، اجرای مستقل file:// ✓
- کپی در download/ManufacturingERP-Demo.html + قابل دانلود از /demo.html روی همان سرور
...
Task ID: 24
Agent: main (Z.ai Code)
Task: نرخ ارزها از sarafi.af + کسر خودکار غیبت از معاش

Work Log:
- کشف API رسمی sarafi.af: GET https://sarafi.af/api/rates — ۲۰ ارز با buy/sell به افغانی؛ قرارداد PKR «هر ۱۰۰۰ کلدار» (226→0.226) با نرخ کراس er-api تأیید شد
- exchange-rate.ts: fromSarafi منبع اصلی (میانگین buy/sell + گارد اعتبار usd∈[10,500] و pkr∈[0.005,5] برای ردِ تغییر قرارداد آینده)؛ er-api و jsDelivr کاپی احتیاطی ماندند
- اعمال خودکار در همهٔ بخش‌ها: داشبورد و گزارشات قبل از خواندن تنظیمات getLiveRates() صدا می‌زنند (کش ۱ ساعته + بدون شکستن مسیر)؛ فروش/POS از قبل نرخ زندهٔ /api/exchange-rate را می‌گیرند → حالا منبع sarafi.af است
- کسر غیبت: فیلدهای absentDays INT و deduction DOUBLE به SalaryPayment در هر ۳ اسکیما (SQLite/MySQL/Vercel) + local-schema (ALTER خودکار SQLite) + mysql-ddl + SYNC_COLUMNS host-setup (ALTER هاست‌های قدیمی)
- src/lib/salary-deduction.ts: jalaliMonthRange (شمسی→میلادی با jalaali-js)، tallyAttendance، deductionPerDay (تنظیم absentDeductionPerDay؛ خالی → ۱/۳۰ معاش)، absenceDeduction
- /api/salaries/preview (GET جدید): salary، شمارش حاضری/غیبت/رخصتی ماه شمسی، نرخ روزانه، کسر کل، مبلغ پیشنهادی، هشدار پرداخت تکراری
- POST /api/salaries: شمارش خودکار روزهای غیبت + ثبت deduction (applyDeduction=false → پرداخت کامل دستی) در رکورد
- local-api handlers/salaries.ts: آینهٔ کامل preview + POST کسر (حالت APK/دمو) + فیکس باگ first-run: بازدیدکنندهٔ تازهٔ دمو به‌جای صفحهٔ ورود، ویزارد «اتصال به هاست» می‌دید — دمو حالا مستقیم ورود
- UI تنظیمات: کارت «کسر خودکار غیبت از معاش» (ورود نرخ روزانه، خالی = ۱/۳۰) + ذخیره در save() + توضیح sarafi.af در تجدید خودکار
- UI معاش: دیالوگ پرداخت با باکس پیش‌نمایش زنده (معاش اساسی، روزهای غیبت، کسر روزانه، مجموع کسر، حاضری/رخصتی، هشدار پرداخت قبلی) و پر شدن خودکار مبلغ پیشنهادی؛ فیش معاش ردیف‌های «روزهای غیبت» و «کسر غیبت» را نشان می‌دهد
- تست: curl — preview بدون تنظیم (9000/30=300×2=600) و با تنظیم 250 (500) ✓؛ POST رکورد absentDays=2/deduction=500 ✓؛ ماه بعد بدون غیبت → کسر ۰ ✓؛ مرورگر هاست — دیالوگ/فیش/تنظیمات/داشبورد ✓؛ مرورگر دمو (env موقت) — ورود ساده admin/admin123 بدون سرور، ثبت غیبت، پیش‌نمایش کسر 400 در حالت محلی ✓
- پاک‌سازی: کارمند/پرداخت/حاضری تست حذف شد؛ تنظیم کسر به حالت خودکار برگشت؛ لاگ: کشف DATABASE_URL به‌صورت متغیر سراسری sandbox ست شده بود (تست دمو با env -u انجام شد)

Stage Summary:
- نرخ ارزها حالا زنده از sarafi.af (مرجع بازار کابل) در تمام سیستم می‌آید: POS/فروش، داشبورد، گزارشات، تنظیمات — با کش ۱ ساعته، کاپی احتیاطی دوگانه و ذخیرهٔ خودکار
- کسر خودکار غیبت: روزهای «غایب» ماه شمسی از حاضری شمرده و به مقدار تنظیم‌شده (یا ۱/۳۰ معاش) از معاش کم می‌شود؛ در پیش‌نمایش، رکورد و فیش معاش ثبت/نمایش داده می‌شود — هم روی هاست و هم در حالت محلی APK/دمو

---
Task ID: 24-test
Agent: main (Z.ai Code)
Task: دیباگینگ و تست عمیق سیستم با دیتای آزمایشی + اعمال تغییرات روی همهٔ صفحه‌ها

Work Log:
- سوئیت تست عمیق end-to-end نوشته و اجرا شد (۴۹ چک خودکار): auth (ورود/نشست/قفل)، نرخ زندهٔ sarafi.af (USD=64.78, PKR=0.23)، تنظیمات (PUT کسر ۲۵۰)، ۳ کارمند آزمایشی، حاضری ۳ کارمند در ماه شمسی جاری، preview کسر (۳×۲۵۰=۷۵۰، غیبت ۱→۲۵۰، بدون غیبت→۰)، ثبت پرداخت با absentDays/deduction، هشدار پرداخت تکراری، ماده/محصول/فرمول/سفارش تولید، فروش دالری/افغانی/کلداری با نرخ خودکار، هزینهٔ چندارزی، داشبورد، گزارشات، انبار، کاربران، ممیزی، و ۴ چک اعتبارسنجی امن (۴۰۴/۴۰۰/۴۰۱) — نتیجه: ۴۹/۴۹ موفق
- باگ بحرانی ۱ کشف و رفع شد: دیتابیس dev ستون‌های missing داشت (User.tokenVersion، SalaryPayment.absentDays/deduction، updatedAt در ۱۲ جدول، Setting.createdAt/updatedAt) → P2022 و ۵۰۰ در auth/me و login؛ ترمیم با ALTER + default ثابت (SQLite در ADD COLUMN فقط default ثابت می‌پذیرد) و سپس prisma db push موفق — دیتا کاملاً حفظ شد
- باگ بحرانی ۲ کشف و رفع شد: «Body is unusable: Body has already been read» در مسیر retry پس از ترمیم اسکیما در login — body حالا یک‌بار در POST خوانده و به handleLogin پاس می‌شود (src/app/api/auth/login/route.ts)
- باگ UX کشف و رفع شد: ماه دیالوگ «پرداخت معاش» خالی بود و پیش‌نمایش کسر غیبت هرگز ظاهر نمی‌شد (نیاز به تایپ دستی 1405-06) → تابع currentJalaliMonth() به src/lib/salary-deduction.ts اضافه و در openPay پیش‌فرض شد؛ حالا باکس پیش‌نمایش (معاش اساسی، روزهای غیبت، کسر روزانه، مجموع کسر قرمز، حاضری/رخصتی) بلافاصله بعد از انتخاب کارمند ظاهر و مبلغ پیشنهادی خودکار پر می‌شود
- تست مرورگر (agent-browser) همهٔ تب‌ها با دیتای آزمایشی: ورود ✓ داشبورد ۸ KPI با فروش چندارزی ✓ کارکنان (جدول، حاضری: ثبت غیبت از UI برای محمود نوری → بلافاصله در preview کسر ظاهر شد ۲×۲۵۰=۵۰۰) ✓ معاش: دیالوگ با پیش‌نمایش زنده، ثبت ۵,۷۵۰ و فیش با ردیف‌های «روزهای غیبت/کسر غیبت» ✓ فروش: نرخ دالری خودکار 64.78 در POS، ثبت بل دالری و فکتور ✓ مالی: صورت سود و زیان با تبدیل زندهٔ $1,609 و Rs100 ✓ گزارشات نمودار ✓ انبار ✓ تولید ✓ تنظیمات: کارت اسعار (تجدید لحظه‌ای → toast «1 دالر = 64.78 AFG»، ذخیره در DB با ratesSource=sarafi.af) + کارت کسر غیبت ✓
- تست موبایل (390×844): منوی همبرگری، KPI دوسطری، footer چسبیده به پایین (bottom=844=viewport) ✓ — صفر خطای کنسول و صفر خطای سرور در همهٔ تب‌ها
- کشف جانبی: یک restore بکاپ قدیمی (مultipart POST /api/admin/backup) حین تست دیتای آزمایشی را پاک کرد — دوباره seed شد؛ ریسک مشابه در تست نهایی با پاک‌سازی معکوس (cleanup.ts به‌جای restore) برطرف شد
- پاک‌سازی کامل دیتای آزمایشی به روش معکوس (فقط رکوردهای «تست»: کارمند ۳، حاضری ۶۰+۱، معاش ۲، فروش ۴+سطرها، تراکنش انبار ۴، فرمول/ماده/محصول/دسته/تأمین‌کننده/مشتری/هزینه، تنظیم کسر→خودکار) — دیتابیس دقیقاً به شمارش‌های اصلی برگشت (۵ کارمند/۷ محصول/۸ فروش/۲۵ حاضری/۵ معاش) و حالت خودکار کسر (۱/۳۰) مجدداً روی دیتای اصلی تست شد (18000→600/روز ✓)
- فایل‌های موقت تست (.test-tmp) حذف شد؛ lint سبز ✓

Stage Summary:
- کل سیستم پس از دیباگینگ عمیق سالم است: ۴۹/۴۹ چک API + همهٔ تب‌ها در دسکتاپ و موبایل بدون خطا؛ تسک ۲۴ (نرخ ارز sarafi.af + کسر خودکار غیبت) اکنون روی همهٔ صفحه‌ها فعال و تست‌شده است
- سه فیکس کد: login retry (body دوباره‌خوانی)، پیش‌فرض ماه شمسی جاری در دیالوگ معاش (currentJalaliMonth)، و همگام‌سازی کامل اسکیمای دیتابیس (۸ ستون missing در ۵ جدول + updatedAt در ۱۲ جدول)
- دیتابیس dev به اسکیمای فعلی همگام شد بدون از دست رفتن دیتا — نصب‌های قدیمی که از v≤۱.۰.۱۸ ارتقا می‌یابند دیگر P2022 نمی‌گیرند

---
Task ID: 25
Agent: main (Z.ai Code)
Task: فیکس سوییچ‌ها + حذف «رمز ادمین را فراموش کرده‌اید؟» + صفحهٔ راه‌اندازی اولیه با آپلود فایل تنظیمات هاست

Work Log:
- باگ سوییچ کشف و رفع شد: translate انگشت سوئیچ فیزیکی است و در RTL در حالت checked انگشت کاملاً از ریل بیرون می‌زد (thumbLeft=32.8 در ریل ۳۶px — لکهٔ سبز خارج از کارت)؛ فیکس در src/components/ui/switch.tsx با واریانت‌های جهت‌آگاه ltr:/rtl: (checked در RTL → translate منفی) — تأیید مرورگری: هر دو حالت inside:true، بدون خطای کنسول
- «رمز ادمین را فوراموش کرده‌اید؟» حذف شد: دکمهٔ صفحهٔ ورود، state forgotOpen و کل کامپوننت ForgotPasswordDialog (+ getAdminResetApi) از src/app/page.tsx پاک شد (۲۱۵ خط)؛ تأیید: snapshot صفحهٔ ورود دیگر دکمه را ندارد
- فایل تنظیمات قابل آپلود در src/lib/host-setup-file.ts: قالب استاندارد JSON (app/type/version/webUrl/db) + buildHostSetupJson + downloadHostSetupJsonFile + parseHostSetupFile که هر سه قالب را می‌فهمد (HostSetup.json، db-connection.txt با خط mysql:// و ssh-*، و bat ویندوزی با استخراج توکن‌های Base64 از بلوک CERTIFICATE) — ۱۱/۱۱ تست roundtrip با پسوردهای خاص (٪ @ = فاصله) پاس شد
- باگ تجزیه‌گر bat حین تست کشف و رفع شد: رجکس بلوک Base64، پیشوند دستور `>> "%VAR%" echo` را هم می‌گرفت → استخراج ران‌های خالص Base64 (≥۲۰ کاراکتر)
- ویزارد ویندوز (setup-wizard.tsx گام ۳): کارت «سریع‌ترین راه — آپلود فایل تنظیمات» در بالای گام هاست؛ آپلود → همهٔ فیلدها (SSH/دیتابیس) خودکار پر، خلاصهٔ سبز (نام دیتابیس/هاست بدون پسورد) + toast؛ فایل نامعتبر → پیام خطا؛ تأیید مرورگری با ?setup=1&desktop=1 و فایل واقعی
- ویزارد اندروید (apk-host-wizard.tsx گام ۲): کارت «فایل تنظیمات دارید؟ همین‌جا آپلودش کنید» — آپلود → آدرس سرور پر و بلافاصله تست خودکار؛ فایل بدون webUrl → پیام راهنما؛ تأیید کامل end-to-end با dev سرور NEXT_PUBLIC_LOCAL_MODE=1: اولین باز شدن → ویزارد → آپلود → پر شدن آدرس → تست موفق → «اتصال ذخیره شد» → صفحهٔ ورود با نشان «در دسترس»
- تنظیمات (دسکتاپ): باکس sky گسترش یافت — ورودی اختیاری «آدرس نسخهٔ وب» + دکمهٔ «دانلود فایل تنظیمات (JSON)» با اعتبارسنجی (toast خطا وقتی فرم خالی، toast موفق با فرم کامل — هر دو تأیید شد)
- تنظیمات (APK): دکمهٔ «فایل تنظیمات» در کارت «اتصال به سرور مرکزی» — فایل JSON با webUrl سرور ذخیره‌شده می‌سازد تا برای نصب‌های جدید فرستاده شود
- سه خطای قدیمی tsc رفع شد: dbFailText با null (setup-wizard)، onClick={finish} که ایونت کلیک را localOnly می‌کرد (setup-wizard — باگ واقعی فلگ «فقط این دستگاه»)، و Row constraint در local-api/handlers/salaries.ts (readCol با LocalAttendance تایپ‌شده + export interface از attendance.ts)
- lint سبز، tsc بدون خطا در src/، dev.log بدون خطا؛ مرورگر: صفر خطای کنسول در همهٔ مسیرها
...
Task ID: 28
Agent: main (Z.ai Code)
Task: «در اول تنظیمات هاست را نخواست… پیام وصل شده را نشان میدهد ولی کار نمیکند… تا هاست تنظیم نشده با رمز و نام کاربری محلی هم وارد نشود… بعد از وصل شدن با رمز و نام کاربری هاست وارد شود و در حالت محلی هم ذخیره شود» — سیاست ورود فقط با کاربران هاست + رفع کامل زنجیرهٔ اتصال (v1.0.28)

Work Log:
- تشخیص ریشهٔ ۱ (ورود محلی از اول): ویزارد گزینهٔ «فقط این دستگاه» داشت (فلگ mfg-setup-local-mode) و گِیت به فلگ‌های localStorage نصب قبلی (منتقل‌شده با migrateLegacyUserData) اعتماد می‌کرد، نه حقیقت سرور
- تشخیص ریشهٔ ۲ («وصل شد ولی کار نمیکند»): بعد از ذخیرهٔ هاست، سرور جاسازی‌شده با DATABASE_URL قدیمی ادامه می‌داد (ری‌استارت اختیاری بود) و حتی بعد از ری‌استارت، روی نصب تازه «admin دمو» (id محلی) با «admin واقعی هاست» (id هاست) برخورد username یکتا می‌کرد → pull کاربر هاست برای همیشه skip و ورود با رمز هاست مستحیل؛ ensureInitialPull هم چون demo-db کاربر داشت skip می‌شد
- گِیت سرور: POST /api/auth/login وقتی DATABASE_URL=mysql نباشد → 451 HOST_NOT_CONFIGURED (ورود محلی حتی دورزدن رندرر بسته است)؛ bootstrap محلی admin در مسیر لاگین دسکتاپ حذف شد (فقط استقرار وب روی هاست خالی ادمین اول می‌سازد)
- پذیرش هاست (adoption) در connection-manager: هویت هاست از ERP_HOST_IDENTITY (main.js از db-connection.txt می‌سازد — در حالت ssh از sshHost:sshPort نه پورت تونل) + متای sync.hostIdentity روی محلی؛ اولین اتصال به یک هاستِ دارای داده → snapshotServerToLocal (جایگزینی کامل محلی با هاست) + بکاپ pre-adoption JSON کنار دیتابیس محلی (dumpLocalTables جدید در sync-engine)؛ دستگاه واقعیِ قدیمی اول runReconnectSync (push تغییرات صف‌شده) بعد اسنپ‌شات
- مارکر device.demo=1 در desktop-assets/demo-db/custom.db و db/custom.db (منبع کپی بیلد) — دیتای دمو هرگز به هاست push نمی‌شود؛ هاست خالی + دستگاه دمو → ensureHostReady({bootstrapFromLocal:false}) یک ادمین تمیز (admin/admin123) مستقیم روی هاست می‌سازد (seededCleanAdmin) و محلی با هاست خالی جایگزین می‌شود
- fallback ورود از هاست: verifyUserOnHost (پینگ ۶ ثانیه‌ای + یافتن کاربر روی MySQL هاست + verifyPassword) → mirrorHostUser (update بر اساس username یا create با id هاست) — آینهٔ محلی تازه و ورود انجام می‌شود؛ اکشن audit جدید login_host_fallback
- گِیت رندرر (first-run.ts + page.tsx): ورودی serverConfigured از /api/system/db-info (حالا عمومی GET برای گِیت)؛ سرور محلی → همیشه ویزارد؛ سرور mysql → برنامه (قطعی موقت با آینهٔ واقعی = ورود آزاد؛ تنظیم‌شده ولی هرگز اسنپ‌شات نداشته → ویزارد با بنر دلیل)؛ فلگ‌های قدیمی منسوخ؛ ?desktop=1 همان قرارداد ویزارد برای تست مرورگر
- ویزارد: کارت «فقط این دستگاه» حذف و گام ۲ تک‌مسیره شد + توضیح سیاست؛ گام ۴ ری‌استارت خودکار ۳ ثانیه‌ای (بدون «بعداً»)؛ تنظیمات: دکمهٔ بازگشت به محلی → «حذف اتصال هاست (بازگشت به صفحهٔ راه‌اندازی)» با توضیح
- بنر وضعیت هاست در صفحهٔ ورود دسکتاپ: تنظیم‌نشده (قرمز + دکمهٔ صفحهٔ راه‌اندازی) و قطع-آینهٔ‌محلی (زرد)
- تست‌ها: گیت ۴۵۱ با curl روی dev و سرور بسته‌بندی‌شده ✓؛ ۱۸/۱۸ چک یکپارچگی پذیرش با دو SQLite (جایگزینی دمو با هاست، عدم نشت دمو به هاست، watermark پایدار، fallback رمز جدید، تعویض هاست) ✓؛ ۱۰/۱۰ تست واحد decideFirstRun ✓؛ مرورگر: ویزارد دسکتاپ گام‌ها ۱→۴ (فقط کارت هاست) + موبایل ۳۹۰px + صفحهٔ ورود با پیام ۴۵۱ — صفر خطای کنسول ✓؛ lint سبز، tsc پاک در src ✓
- بیلد و انتشار v1.0.28: bump چهار فایل (versionCode 13)؛ app.apk 1,285,456B (aapt2: versionName=1.0.28)؛ دسکتاپ 600M win-unpacked (رشته‌های sync.hostIdentity/HOST_NOT_CONFIGURED/1.0.28 در باندل)؛ smoke بسته: GET / 200 + login 451 + db-info 1.0.28/19 جدول ✓؛ Setup.exe 172,642,091B (UTF-16 1.0.28.0)؛ Portable.zip 271,901,476B

Stage Summary:
- سیاست جدید فعال است: تا هاست تنظیم و اعمال نشود هیچ ورودی وجود ندارد؛ بعد از اتصال، ورود با کاربران هاست است و دیتای هاست روی هر دستگاه آینه می‌شود (آفلاین هم کار می‌کند)
- باگ «وصل شد ولی کار نمیکند» در سه لایه رفع شد: اعمال قطعی اتصال با ری‌استارت خودکار + پذیرش کامل دیتای هاست در اولین اتصال + fallback تأیید کاربر از هاست
- v1.0.28 منتشر شد: apk sha256 2deb1574… / Setup cf50d8f8… / Portable b32ec68b…
