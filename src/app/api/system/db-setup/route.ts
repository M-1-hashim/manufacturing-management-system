 ...
 *
 * GET              → وضعیت جدول‌ها/کاربران روی هاست
 * POST create      → ساخت خودکار جدول‌های گمشده (۱۹ جدول) + بوت‌استرپ کاربران
 * POST migrate     → انتقال کامل دیتای این دستگاه به هاست (upsert همهٔ جدول‌ها)
 *
 * نکته: «create» وقتی هاست خالی است کاربران/تنظیمات محلی را هم کپی می‌کند
 * تا بلافاصله بعد از اتصال، ورود به سیستم کار کند.
 *
 * فقط ادمین (نشست معتبر) — در ویزارد راه‌اندازی اولیه نیازی به این API نیست،
 * چون همان کار به‌صورت خودکار در شروع سرور انجام می‌شود.
 */
